...
def get_travelers_items(request, list_id): #What goes into the parameters can be used for the function.
    print(
        'User', f"{request.user.firt_name}{request.user.last_name}")
    if request.method =='GET':
        travelers_list = List.object.filter(list_id=list_id)
        serializer = ListSerializer(travelers_list, many = True)
        return Response(serializer.data)
    if request.method =='POST':
        serializer = ListSerializer( data=request.data)
        if serializer.is_valid():
            serializer.save(user=request.user, list_id=list_id)
